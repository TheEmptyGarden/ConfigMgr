# Name:         Install-O365ProPlus64
# Description:  Installs O365ProPlus64
...
try
{
  Function Main
  {
    ...
    #Install O365ProPlus64
    InstallO365ProPlus64
  ...
  Function SetGlobalEnvVariables
  {
    ...
    $Script:LoggingPath = 'c:\Windows\Temp\Install-O365ProPlus64.log'
...
Function FindProject
  {
    ...
    $script:x86Project = ((Get-ChildItem "HKLM:\Software\Microsoft\Windows\CurrentVersion\Uninstall") |
    ...
    $script:x64Project = ((Get-ChildItem "HKLM:\Software\Wow6432Node\Microsoft\Windows\CurrentVersion\Uninstall") |
        Where-Object { $_.GetValue( "DisplayName" ) -like "*$script:ProjectProgram*" } ).Length -gt 0;
...
Function FindVisio
  {
    ...
    $script:x86Visio = ((Get-ChildItem "HKLM:\Software\Microsoft\Windows\CurrentVersion\Uninstall") |
       Where-Object { $_.GetValue( "DisplayName" ) -like "*$script:VisioProgram*" } ).Length -gt 0;

    LogTraceMessage "Visio x86 is $x86Visio"
    Write-Verbose "Visio x86 is $x86Visio"

    $script:x64Visio = ((Get-ChildItem "HKLM:\Software\Wow6432Node\Microsoft\Windows\CurrentVersion\Uninstall") |
        Where-Object { $_.GetValue( "DisplayName" ) -like "* $script:VisioProgram*" } ).Length -gt 0;
  ...
  Function RemoveOffice
  {
    ...
    if (Test-Path 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\{90140000-006D-0409-1000-0000000FF1CE}')
      ...
      if (((Get-ItemProperty -Path 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\{90140000-006D-0409-1000-0000000FF1CE}').DisplayName) -like "*Office*")
        ...
        Write-Verbose "Removing Office in Uninstall registry started"
  ...
  Function InstallO365ProPlus64
  {
    # Function Started
    LogTraceMessage "*** Function InstallO365ProPlus64 Started ***"
    Write-Verbose "*** Function InstallO365ProPlus64 Started ***"

    $Script:proc = (Start-Process -FilePath "setup.exe" -ArgumentList "/configure Office365Install64Configuration.xml" -PassThru)

    LogTraceMessage "InstallO365ProPlus64 ID =  $($Script:proc.ID)"
    Write-Verbose "InstallO365ProPlus64 ID =  $($Script:proc.ID)" 
      
    $Script:proc.WaitForExit()
    $Script:ExitCode = $Script:proc.ExitCode
    
    LogTraceMessage "*** Function InstallO365ProPlus64 Ended ***"
    Write-Verbose "*** Function InstallO365ProPlus64 Ended ***"
  } 

Function InstallVisio64
  {
  # Function Started
  LogTraceMessage "*** Function InstallO365Visio64 Started ***"
  Write-Verbose "*** Function InstallO365Visio64 Started ***"
  ...
  If ($script:x86Visio -eq $True -or $script:x64Visio -eq $True)
    ...
    $Script:proc = (Start-Process -FilePath "setup.exe" -ArgumentList "/configure Visio365Install64Configuration.xml" -PassThru)

    
    LogTraceMessage "InstallO365Visio64 ID =  $($Script:proc.ID)"
    Write-Verbose "InstallO365Visio64 ID =  $($Script:proc.ID)" 
    
    $Script:proc.WaitForExit()
    $Script:ExitCode = $Script:proc.ExitCode

    LogTraceMessage "*** Function InstallO365Visio64 Ended ***"
    Write-Verbose "*** Function InstallO365Visio64 Ended ***"
    }
  }

Function InstallProject64
  {
  # Function Started
