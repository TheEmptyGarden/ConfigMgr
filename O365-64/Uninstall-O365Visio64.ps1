...
try
{
  ...
  Function UninstallO365Visio64
  {
    # Function Started
    LogTraceMessage "*** Function UnInstallO365Visio64 Started ***"
    Write-Verbose "*** Function UnInstallO365Visio64 Started ***"

    $Script:proc = (Start-Process -FilePath "setup.exe" -ArgumentList "/configure Visio365Uninstall64Configuration.xml" -PassThru)

    
    LogTraceMessage "UnInstallO365Visio64 ID =  $($Script:proc.ID)"
    Write-Verbose "UnInstallO365Visio64 ID =  $($Script:proc.ID)" 
