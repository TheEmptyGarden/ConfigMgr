# Name:         Install-O365Skype64
# Description:  Installs O365Skype64
...
try
{
  ...
  Function SetGlobalEnvVariables
  {
    ...
    $Script:LoggingPath = 'c:\Windows\Temp\Install-O365Skype64.log'
...
Function RemovePreviousClickToRunx86
  {
    ...
    if (Test-Path 'HKLM:\Software\Microsoft\Office\ClickToRun\Configuration')
    {
      ...
      If (((Get-ItemProperty -path 'HKLM:\Software\Microsoft\Office\ClickToRun\Configuration').Platform) -eq "x86")
      {
        ...
        $Script:proc = (Start-Process -FilePath "setup.exe" -ArgumentList "/configure Skype365UninstallALL64Configuration.xml" -PassThru)
  ...
  Function InstallO365Skype64
  {
    # Function Started
    LogTraceMessage "*** Function InstallO365Skype64 Started ***"
    Write-Verbose "*** Function InstallO365Skype64 Started ***"
    ...
    if (Test-Path 'HKLM:\Software\Microsoft\Office\ClickToRun\Configuration')
      ...
      if (((Get-ItemProperty -path 'HKLM:\Software\Microsoft\Office\ClickToRun\Configuration').Platform) -eq "x64")
        ...
        $Script:proc = (Start-Process -FilePath "setup.exe" -ArgumentList "/configure Skype365Install64Configuration.xml" -PassThru)
        }
      }
    ...
    LogTraceMessage "InstallO365Skype64 ID =  $($Script:proc.ID)"
    Write-Verbose "InstallO365Skype64 ID =  $($Script:proc.ID)" 
