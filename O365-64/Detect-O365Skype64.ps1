# Name:         Detect-O365Skype64
# Description:  Detects O365Skype64
...
try
{
  ...
  Function SetGlobalEnvVariables
  {
    ...
    $Script:LoggingPath = 'c:\Windows\Temp\Detect-O365Skype64.log'
  ...
  Function DetectO365Skype64
  {
    # Function Started
    LogTraceMessage "*** Function DetectO365Skype64 Started ***"
    Write-Verbose "*** Function DetectO365Skype64 Started ***"
    ...
    $Script:AppDetect = if (Test-Path 'HKLM:\Software\Microsoft\Office\ClickToRun\Configuration')
    {
      if(
        ...
        -and ((Get-ItemProperty -path 'HKLM:\Software\Microsoft\Office\ClickToRun\Configuration').Platform) -eq "x64" `
        ...
        -and (Test-Path "$env:Programfiles\Microsoft Office\root\Office16\lync.exe")
        )
      {
        $True
      }
    ...
    LogTraceMessage "DetectO365Skype64 is $Script:AppDetect"
    Write-Verbose "DetectO365Skype64 is $Script:AppDetect"

    LogTraceMessage "*** FunctionDetectO365Skype64 Finished ***"
    Write-Verbose "*** Function DetectO365Skype64 Finished ***"
   }
