...
try
{
  ...
  Function SetGlobalEnvVariables
  {
    ...
    $Script:LoggingPath = 'c:\Windows\Temp\Uninstall-O365ProPlus64.log'
  ...
  Function UninstallO365ProPlus64
  {
    # Function Started
    LogTraceMessage "*** Function UnInstallO365ProPlus64 Started ***"
    Write-Verbose "*** Function UnInstallO365ProPlus64 Started ***"

    $Script:proc = (Start-Process -FilePath "setup.exe" -ArgumentList "/configure Office365UninstallConfiguration.xml" -PassThru)

    
    LogTraceMessage "UnInstallO365ProPlus64 ID =  $($Script:proc.ID)"
    Write-Verbose "UnInstallO365ProPlus64 ID =  $($Script:proc.ID)" 
