# Name:         Install-O365Visio64
# Description:  Installs O365Visio64
...
try
{
  ...
  Function SetGlobalEnvVariables
  {
    ...
    $Script:LoggingPath = 'c:\Windows\Temp\Install-O365Visio64.log'
  ...
  Function RemovePreviousClickToRunx86
  {
    ...
    if (Test-Path 'HKLM:\Software\Microsoft\Office\ClickToRun\Configuration')
    {
      ...
      If (((Get-ItemProperty -path 'HKLM:\Software\Microsoft\Office\ClickToRun\Configuration').Platform) -eq "x86")
      {
        ...
        $Script:proc = (Start-Process -FilePath "setup.exe" -ArgumentList "/configure Visio365UninstallALL64Configuration.xml" -PassThru)
