# Name:         Detect-O365Project32
# Description:  Detects O365Project32
...
try
{
  ...
  Function SetGlobalEnvVariables
  {
    ...
    $Script:LoggingPath = 'c:\Windows\Temp\Detect-O365Project32.log'
  ...
  Function DetectO365Project32
  {
    # Function Started
    LogTraceMessage "*** Function DetectO365Project32 Started ***"
    Write-Verbose "*** Function DetectO365Project32 Started ***"
    ...
    If ($OSArchitecture -eq '64-bit')
        {
        $ClickToRunProject = Test-Path "${env:ProgramFiles(x86)}\Microsoft Office\root\Office16\WINPROJ.EXE"
        LogTraceMessage "ClickToRunProject = $ClickToRunProject"
        Write-Verbose "ClickToRunProject = $ClickToRunProject" 
        }
    Elseif ($OSArchitecture -eq '32-bit')
        {
        $ClickToRunProject = Test-Path "$env:ProgramFiles\Microsoft Office\root\Office16\WINPROJ.EXE"
        LogTraceMessage "ClickToRunProject = $ClickToRunProject"
        Write-Verbose "ClickToRunProject = $ClickToRunProject" 
        }

    $Script:AppDetect = if ($ClickToRun = $True -and ($ClickToRunVersionToRerport -ge '16.0.10325.20082' -or $ClickToRunClientVersionToReport -ge '16.0.10325.20082') -and $ClickToRunPlatform -eq "x86" -and $ClickToRunProductReleaseIDs -like "*Project*" -and $ClickToRunProject -eq $True)
    ...
    LogTraceMessage "DetectO365Project32 is $Script:AppDetect"
    Write-Verbose "DetectO365Project32 is $Script:AppDetect"

    LogTraceMessage "*** FunctionDetectO365Project32 Finished ***"
    Write-Verbose "*** Function DetectO365Project32 Finished ***"
   }
