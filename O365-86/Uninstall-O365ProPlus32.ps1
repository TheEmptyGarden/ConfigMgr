...
try
{
  ...
  Function UninstallO365ProPlus32
  {
    # Function Started
    LogTraceMessage "*** Function UnInstallO365ProPlus32 Started ***"
    Write-Verbose "*** Function UnInstallO365ProPlus32 Started ***"

    $Script:proc = (Start-Process -FilePath "setup.exe" -ArgumentList "/configure Office365UninstallConfiguration.xml" -PassThru)

    
    LogTraceMessage "UnInstallO365ProPlus32 ID =  $($Script:proc.ID)"
    Write-Verbose "UnInstallO365ProPlus32 ID =  $($Script:proc.ID)" 
