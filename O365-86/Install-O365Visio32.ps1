  ...
  Function RemovePreviousClickToRunx64
  {
    ...
    if (Test-Path 'HKLM:\Software\Microsoft\Office\ClickToRun\Configuration')
    {
      ...
      If (((Get-ItemProperty -path 'HKLM:\Software\Microsoft\Office\ClickToRun\Configuration').Platform) -eq "x64")
      {
        ...
        $Script:proc = (Start-Process -FilePath "setup.exe" -ArgumentList "/configure Visio365UninstallALL32Configuration.xml" -PassThru)
    ...
    LogTraceMessage "*** Function RemovePreviousClickToRunx64 Ended ***"
    Write-Verbose "*** Function RemovePreviousClickToRunx64 Ended ***"
  }


  Function InstallO365Visio32
  {
    # Function Started
